...
function activity_description(UserActivity $activity): string
{
    ...
    return match ($activity->action) {
        'post_created' => 'Created a discussion: '.($target ? "<a href='".route('posts.show', $target)."' class='font-bold hover:text-accent transition'>".e($target->title).'</a>' : '<strong>a deleted discussion</strong>'),

        'comment_created' => 'Commented on '.($target?->post ? "<a href='".route('posts.show', $target->post)."' class='font-bold hover:text-accent transition'>".e($target->post->title).'</a>' : '<strong>a discussion</strong>'),

        'vote_up' => 'Upvoted a '.($target instanceof Post ? 'discussion' : 'comment'),

        'vote_down' => 'Downvoted a '.($target instanceof Post ? 'discussion' : 'comment'),

        'authors_pick_selected' => "Highlighted a comment as Author's Pick",
