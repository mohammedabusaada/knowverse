...
<div
    x-data="{ seen: {{ $isUnread ? 'false' : 'true' }} }"
    @if($isUnread)
    ...
    class="relative flex items-start gap-4 p-5 transition-all duration-300 
           {{ $isUnread ? 'bg-aged/50' : 'hover:bg-aged/20 bg-paper' }}">

    {{-- Unread Indicator --}}
    @if($isUnread)
        <div class="absolute left-0 top-0 bottom-0 w-1 bg-ink"></div>
    @endif

    {{-- Visual Icon rendered from the Presenter --}}
    <div class="flex-shrink-0 pt-0.5">
        <div class="flex items-center justify-center w-8 h-8 rounded-sm border border-rule {{ $isUnread ? 'bg-paper shadow-sm' : 'bg-aged opacity-60' }}">
