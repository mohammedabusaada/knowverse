@extends('layouts.app')

@section('content')
{{-- Reading Progress Bar (Monochrome + subtle blue accent) --}}
<div class="fixed top-0 left-0 w-full h-1 z-50">
    <div
        x-data="{
            percent: 0,
            updateProgress() {
                let winScroll = document.body.scrollTop || document.documentElement.scrollTop;
                let height = document.documentElement.scrollHeight - document.documentElement.clientHeight;
                this.percent = height ? (winScroll / height) * 100 : 0;
            }
        }"
        x-init="
            updateProgress();
            window.addEventListener('scroll', () => updateProgress())
        "
        :style="'width: ' + percent + '%'"
        class="h-full bg-blue-600/80 dark:bg-blue-500/80 transition-all duration-150"
    ></div>
</div>

<div class="max-w-7xl mx-auto px-4 py-8 lg:flex lg:gap-8">

    {{-- Main --}}
    <div class="lg:w-3/4">

        {{-- Breadcrumb + Title --}}
        <div class="mb-6">
            <nav class="flex text-xs uppercase tracking-widest text-gray-500 dark:text-gray-400 mb-4">
                <a href="{{ route('posts.index') }}" class="hover:text-blue-600 dark:hover:text-blue-400 transition">
                    Posts
                </a>
                <span class="mx-2 text-gray-300 dark:text-gray-700">/</span>
                <span class="text-gray-400 dark:text-gray-500">Discussion</span>
            </nav>

            <h1 class="text-2xl md:text-3xl font-semibold tracking-tight text-gray-900 dark:text-gray-100 leading-snug">
                {{ $post->title }}
            </h1>
        </div>

        {{-- Post Card --}}
        <div class="flex gap-4 md:gap-6
                    bg-white dark:bg-gray-900
                    border border-gray-200 dark:border-gray-800
                    rounded-2xl p-4 md:p-6
                    shadow-sm">

            {{-- Left rail --}}
            <div class="flex flex-col items-center pt-1">
                <x-post-vote :post="$post" />

                <button
                    title="Bookmark"
                    class="mt-4 inline-flex items-center justify-center
                           w-9 h-9 rounded-lg
                           text-gray-400 hover:text-gray-900
                           dark:hover:text-gray-100
                           hover:bg-gray-100 dark:hover:bg-gray-800
                           transition"
                >
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"/>
                    </svg>
                </button>
            </div>

            {{-- Content --}}
            <div class="flex-1 overflow-hidden">

                {{-- Meta row --}}
                <div class="flex flex-wrap items-center gap-x-2 gap-y-2 text-sm
                            border-b border-gray-200 dark:border-gray-800 pb-4 mb-6">

                    <x-user-avatar :src="$post->user->profile_picture_url" size="xs" />
                    <x-user-hover-card :user="$post->user" />

                    <span class="text-gray-300 dark:text-gray-700">•</span>
                    <span class="text-gray-500 dark:text-gray-400">
                        {{ $post->created_at->diffForHumans() }}
                    </span>

                    {{-- Reading Time --}}
                    @php
                        $wordCount = str_word_count(strip_tags($post->body));
                        $minutes = max(1, ceil($wordCount / 200));
                    @endphp
                    <span class="text-gray-300 dark:text-gray-700">•</span>
                    <span class="text-gray-500 dark:text-gray-400 flex items-center gap-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                  d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                        {{ $minutes }} min read
                    </span>

                    <div class="ml-auto flex items-center gap-2">
                        <span class="hidden sm:flex items-center gap-1 text-gray-500 dark:text-gray-400">
                            <x-icons.eye class="w-4 h-4" />
                            {{ number_format($post->view_count) }}
                        </span>

                        {{-- Share --}}
                        <div
                            x-data="{
                                copied: false,
                                share() {
                                    navigator.clipboard.writeText(window.location.href);
                                    this.copied = true;
                                    setTimeout(() => this.copied = false, 2000);
                                }
                            }"
                            class="relative"
                        >
                            <button
                                @click="share"
                                class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg
                                       bg-gray-100 hover:bg-gray-200
                                       dark:bg-gray-800 dark:hover:bg-gray-700
                                       text-gray-700 dark:text-gray-200
                                       border border-gray-200 dark:border-gray-700
                                       transition text-xs font-semibold"
                            >
                                <svg x-show="!copied" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                          d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"/>
                                </svg>
                                <svg x-show="copied" class="w-4 h-4 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                          d="M5 13l4 4L19 7"/>
                                </svg>
                                <span x-text="copied ? 'Copied' : 'Share'"></span>
                            </button>
                        </div>
                    </div>
                </div>

                {{-- Image --}}
                @if ($post->image)
                    <div class="mb-6">
                        <img
                            src="{{ asset('storage/'.$post->image) }}"
                            class="rounded-xl w-full object-cover max-h-[520px]
                                   border border-gray-200 dark:border-gray-800"
                            alt="Post image"
                        >
                    </div>
                @endif

                {{-- Body --}}
                <article class="prose prose-lg dark:prose-invert max-w-none mb-8 leading-relaxed">
                    <x-markdown :text="$post->body" />
                </article>

                {{-- Footer: tags + admin actions --}}
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3
                            pt-4 border-t border-gray-200 dark:border-gray-800">

                    @if ($post->tags->count())
                        <div class="flex flex-wrap gap-2">
                            @foreach($post->tags as $tag)
                                <a
                                    href="/search?q={{ $tag->name }}&type=tags"
                                    class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full
                                           text-xs font-semibold
                                           bg-gray-100 text-gray-800 border border-gray-200
                                           hover:border-blue-300 hover:text-blue-700
                                           dark:bg-gray-950 dark:text-gray-200 dark:border-gray-800
                                           dark:hover:border-blue-700 dark:hover:text-blue-400
                                           transition"
                                >
                                    <span class="text-blue-600/80 dark:text-blue-400/80">#</span>{{ $tag->name }}
                                </a>
                            @endforeach
                        </div>
                    @endif

                    @can('update', $post)
                        <div class="flex items-center gap-3">
                            <a
                                href="{{ route('posts.edit', $post) }}"
                                class="text-xs font-semibold uppercase tracking-wider
                                       text-gray-500 hover:text-blue-600
                                       dark:text-gray-400 dark:hover:text-blue-400 transition"
                            >
                                Edit
                            </a>

                            <form action="{{ route('posts.destroy', $post) }}" method="POST" onsubmit="return confirm('Delete post?');">
                                @csrf @method('DELETE')
                                <button
                                    class="text-xs font-semibold uppercase tracking-wider
                                           text-gray-500 hover:text-red-600
                                           dark:text-gray-400 dark:hover:text-red-400 transition"
                                >
                                    Delete
                                </button>
                            </form>
                        </div>
                    @endcan
                </div>

            </div>
        </div>

        {{-- Discussion Section --}}
        <div class="mt-8">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-5 flex items-center gap-2">
                <x-icons.chat class="w-5 h-5 text-gray-700 dark:text-gray-300" />
                Discussion
                <span class="text-sm font-semibold text-gray-500 dark:text-gray-400">
                    ({{ $post->allComments()->count() }})
                </span>
            </h2>

            @auth
                <div class="mb-8 bg-white dark:bg-gray-900
                            border border-gray-200 dark:border-gray-800
                            rounded-2xl p-5 sm:p-6 shadow-sm">
                    <form action="{{ route('comments.store') }}" method="POST">
                        @csrf
                        <input type="hidden" name="post_id" value="{{ $post->id }}">

                        <x-textarea
                            name="body"
                            rows="4"
                            class="w-full border-gray-300 focus:ring-blue-500"
                            placeholder="What are your thoughts?"
                            required
                        ></x-textarea>

                        <div class="flex justify-end mt-4">
                            <x-button primary>
                                Post Comment
                            </x-button>
                        </div>
                    </form>
                </div>
            @else
                <div class="p-6 text-center
                            bg-white dark:bg-gray-900
                            rounded-2xl border border-dashed border-gray-300 dark:border-gray-800
                            mb-8">
                    <p class="text-gray-600 dark:text-gray-400">
                        Want to join the conversation?
                        <a href="{{ route('login') }}" class="font-semibold text-blue-600 dark:text-blue-400 hover:underline">
                            Log in
                        </a>
                        to comment.
                    </p>
                </div>
            @endauth

            <div class="space-y-6">
                @foreach ($comments as $comment)
                    <x-comment :comment="$comment" />
                @endforeach
            </div>
        </div>
    </div>

    {{-- Sidebar --}}
    <div class="hidden lg:block lg:w-1/4">
        <div class="sticky top-24 space-y-6">

            {{-- About --}}
            <div class="bg-white dark:bg-gray-900
                        border border-gray-200 dark:border-gray-800
                        rounded-2xl p-6 shadow-sm">
                <h3 class="text-xs font-semibold uppercase tracking-widest text-gray-700 dark:text-gray-300 mb-4">
                    About the Author
                </h3>

                <div class="flex items-center gap-3 mb-4">
                    <x-user-avatar :src="$post->user->profile_picture_url" size="lg" />
                    <div class="min-w-0">
                        <p class="font-semibold text-gray-900 dark:text-gray-100 leading-tight truncate">
                            {{ $post->user->display_name }}
                        </p>
                        <p class="text-xs text-gray-500 dark:text-gray-400 truncate">
                            {{ '@'.$post->user->username }}
                        </p>
                    </div>
                </div>

                <p class="text-sm text-gray-600 dark:text-gray-400 line-clamp-3 mb-4">
                    {{ $post->user->bio ?? 'This user hasn\'t shared a bio yet.' }}
                </p>

                <a
                    href="{{ route('profiles.show', $post->user->username) }}"
                    class="block text-center py-2 rounded-xl
                           bg-gray-100 hover:bg-gray-200
                           dark:bg-gray-800 dark:hover:bg-gray-700
                           border border-gray-200 dark:border-gray-700
                           text-sm font-semibold text-gray-800 dark:text-gray-100
                           transition"
                >
                    View Profile
                </a>
            </div>

            {{-- Rules --}}
            <div class="p-4 text-[11px]
                        text-gray-600 dark:text-gray-400
                        bg-white dark:bg-gray-900
                        rounded-xl border border-gray-200 dark:border-gray-800">
                <h4 class="font-semibold uppercase tracking-wider mb-2 text-gray-700 dark:text-gray-300">
                    Rules
                </h4>
                <ul class="list-disc ml-4 space-y-1">
                    <li>Be respectful.</li>
                    <li>No spam.</li>
                    <li>Cite sources.</li>
                </ul>
            </div>

        </div>
    </div>

</div>
@endsection
