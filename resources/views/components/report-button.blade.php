...
@if(!$isOwner)
        ...
        <button
            ...
            @click.stop="window.dispatchEvent(new CustomEvent('open-modal', { detail: 'report-{{ $type }}-{{ $id }}' }))"
        >
            Report {{ ucfirst($type) }}
        </button>
            ...
            <x-modal name="report-{{ $type }}-{{ $id }}" maxWidth="md">
                <form method="POST" action="{{ route('reports.store') }}" class="p-6 bg-paper border border-rule">
                    ...
                    <div class="space-y-5">
                        {{-- Reason Select --}}
                        <div>
                            <label class="block font-mono text-[10px] uppercase tracking-widest text-muted mb-2 font-bold">Primary Reason</label>
                        ...
                        {{-- Additional Details --}}
                        <div>
                            <label class="block font-mono text-[10px] uppercase tracking-widest text-muted mb-2 font-bold">Additional Context (Optional)</label>
                            <textarea
                                name="reason"
                                rows="3"
