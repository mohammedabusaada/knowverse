    ...
    <div class="flex w-full md:w-1/2 justify-center items-center bg-white px-8 py-12">
        <div class="max-w-md w-full bg-white p-10 rounded-3xl shadow-2xl border border-gray-100 hover:shadow-gray-300/40 transition-shadow duration-300">
            ...
            <form action="{{ route('login') }}" method="POST" class="space-y-6" novalidate>
                ...
                <div class="flex items-center justify-between text-sm mt-2">
                    <label class="flex items-center text-gray-700 cursor-pointer">
                        <input type="checkbox" name="remember" class="h-4 w-4 text-gray-700 border-gray-300 rounded mr-2 focus:ring-gray-400" />
                        Remember me
                    </label>
                    <a href="{{ route('password.request') }}" class="text-gray-600 hover:text-gray-900 hover:underline font-medium transition">
                        Forgot password?
                    </a>
                </div>

                <button type="submit" class="w-full bg-[#1a1a1a] hover:bg-gray-900 text-white font-semibold py-3 rounded-full shadow-md hover:shadow-lg focus:ring-4 focus:ring-gray-300 transition-transform hover:-translate-y-0.5">
                    Sign In
                </button>
            </form>
            ...
            <p class="mt-8 text-center text-gray-600 text-sm">
                ...
                <a href="{{ route('register') }}" class="text-gray-900 font-semibold hover:underline hover:text-black transition-colors">
                    Create one
                </a>
            </p>
        </div>
